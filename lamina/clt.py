...
class Laminate:
    ...
    @property
    def K_all(self):
        """Lazy evaluation of failure-specific transformation matrices"""
        if getattr(self, '_K_all', None) is None:
            Q = self.material.Q()
            c2 = self.c * self.c
            s2 = self.s * self.s
            cs = self.c * self.s
    ...
    @property
    def K_all_z(self):
        """Lazy evaluation of failure-specific depth matrices"""
        if getattr(self, '_K_all_z', None) is None:
            zk = self.z_coords[1:]
            zk_1 = self.z_coords[:-1]
            z_mids = (zk + zk_1) / 2
            self._K_all_z = self.K_all * z_mids[:, np.newaxis, np.newaxis]
        return self._K_all_z
